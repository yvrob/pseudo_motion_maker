...
class Restart_File:
    ...
    def __init__(self, path_to_file=None, snapshots=None, materials=None):
        ...
        self.snapshots = dict()
        test = [
            not isinstance(i, type(None)) for i in [path_to_file, snapshots, materials]
        ]
        if sum(test) != 1:
    ...
    def __repr__(self):
        ...
        s = "Restart file: {}, snapshots ({} points):\n".format(
            self.path_to_file, len(self.snapshots)
        )
        for i in self.snapshots:
            s += "\t{}: BU = {:.2f} MWd/kg, time  = {:.2f} days, {} materials\n".format(
                i, self._burnups[i], self._times[i], len(self.snapshots[i])
            )
        s += "\tWritten: {}".format(os.path.exists(self.path_to_file))
        return s

    def read_restart(self, passes=False):
        ...
        with open(self.path_to_file, mode="rb") as file:  # b is important -> binary
            while True:
                ...
                if (
                    len(self._burnups) == 0
                    or mat.bu_global != self._burnups[list(self._burnups.keys())[-1]]
                ):
                    # Iterate step
                    current_step += 1
    ...
    def write_binary(self, snapshot_ids=None, material_names=None, passes=False):
        ...
        print(
            "Writing snapshots to binary {} in {}".format(
                snapshot_ids, self.path_to_file
            )
        )
        ...
        for i_step, s in enumerate([self.snapshots[j] for j in snapshot_ids]):
            ...
            print(
                "\tProcessing snapshot {} with {} materials".format(
                    snapshot_ids[i_step], len(material_names)
                )
            )

            for mat in materials:
                contents.append(mat.to_binary(passes))
    ...
    def write_text(self, snapshot_ids=None, material_names=None, name_out=None):
        if "wrk_dd" in self.path_to_file:
            domain_ID = int(self.path_to_file.split("dd")[-1])
        else:
            domain_ID = 0
        ...
        for i_step, s in enumerate([self.snapshots[j] for j in snapshot_ids]):
            ...
            print(
                "\tWriting snapshot {} with {} materials".format(
                    snapshot_ids[i_step], len(material_names)
                )
            )
            ...
            if domain_ID == 0:
                ...
                path_out = "{}_dd{}_{}.txt".format(
                    prefix, domain_ID, snapshot_ids[i_step]
                )
...
class Material:
    ...
    def __repr__(self):
        ...
        top = sorted(
            self.nuclides.items(), key=lambda nuc: nuc[1]["adens"], reverse=True
        )
        s = "{}, adens: {:.2E}, bu: {:.2E}, nnuc: {}, top 5: {}".format(
            self.name,
            self.adens,
            self.bu,
            self.nnuc,
            " ".join([translate(i[0]) for i in top[:5]]),
        )
        return s

    def read(self, file, passes=False):
        ...
        if passes:
            self.passes = struct.unpack("q", file.read(8))[
                0
            ]  # Number of passes, only with discrete motion
    ...
    def to_binary(self, passes=False):
        ...
        if passes:
            content += struct.pack("q", self.passes)
...
if __name__ == "__main__":
    ...
    ## Quick check
    print(
        "\tCheck: {} materials in restart file, {} extracted, {} removed".format(
            len(restart1.extract_snapshot(-1)),
            len(materials1),
            len(restart1.extract_snapshot(-1)) - len(materials1),
        )
    )
    ...
    bu_global = list(materials1.values())[
        0
    ].bu_global  # take whatever material which is in materials1
